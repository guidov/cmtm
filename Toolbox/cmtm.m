...
%
%
%required files: cohconf.m, cohbias.m, cohbias.mat
%
%Peter Huybers
%MIT, 2003
%phuyber@mit.edu
...
for iter=1:confn;
  ...
  xs=xs-mean(xs); 
  ys=ys-mean(ys);
  [si, ciph(iter,:), phi(iter,:)]=cmtmQSR(xs/std(xs),ys/std(ys),dt,NW,0,0,0);
end;
...
if qplot==1,
  ...
  figure(gcf); clf;
  subplot(211); hold on;
  h=plot(s,c,'r');  set(h,'linewidth',1.5);
  h=ylabel('coherence'); font(h,14);
  h=xlabel('frequency (1/kyr)'); font(h,14);
  font(gca,13);
  plot(s,ci,'k--');
  pl=find(c>ci(1));
  ...
  %phase
  subplot(212); hold on;
  h=plot(s,ph,'r');  set(h,'linewidth',1.5);
  if confn>0,
    col=[.9 .9 .9];
