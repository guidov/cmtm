...
%   nw     - Time bandwidth product, acceptable values mailPHare
...
%Iteration to determine adaptive weights:    
   if k>1,
     ...
     a    = sig2*(1-V);
     count=0;
   while sum(abs(P-P1)/nfft)>tol | count<1,
     count=count+1;
...
set(h,'edgecolor',c);
h=plot(s,P,'r'); set(h,'linewidth',2);
...
h=xlabel('frequency (cycles/deltat)'); font(h,17);
h=ylabel('power density (units^2/cycle/deltat)'); font(h,17);
end;
